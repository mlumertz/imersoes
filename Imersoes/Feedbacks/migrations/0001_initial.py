# -*- coding: utf-8 -*-
# Generated by Django 1.10.5 on 2017-01-09 21:39
from __future__ import unicode_literals

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='Cliente',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('Nome', models.CharField(max_length=16)),
                ('SobreNome', models.CharField(max_length=16)),
                ('Email', models.EmailField(max_length=254)),
                ('Observacoes', models.TextField()),
                ('WebKey', models.CharField(max_length=32)),
                ('Status', models.BooleanField(verbose_name=False)),
                ('Orientador', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
        ),
        migrations.CreateModel(
            name='Indicado',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('Nome', models.CharField(max_length=16)),
                ('SobreNome', models.CharField(max_length=16)),
                ('Email', models.EmailField(max_length=254)),
                ('WebKey', models.CharField(max_length=32)),
                ('Status', models.BooleanField(verbose_name=False)),
                ('QuemIndicou', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='Feedbacks.Cliente')),
            ],
        ),
    ]
